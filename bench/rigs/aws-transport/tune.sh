...
SSH="ssh -i $HOME/.ssh/kvbench.pem -o StrictHostKeyChecking=accept-new -o ConnectTimeout=10 ec2-user"
SYSCTL="$(git rev-parse --show-toplevel)/bench/rigs/sysctl-esnet.conf"

for host in "$A_PUB" "$B_PUB"; do
  echo "[tune] $host"
  scp -i $HOME/.ssh/kvbench.pem -o StrictHostKeyChecking=accept-new "$SYSCTL" "ec2-user@$host:/tmp/sysctl-esnet.conf"
