...
mk_cmd() {  # $1 = job name; reads the CURRENT env (incl. BASELINE_ONLY)
  ...
  local v
  for v in MAX_MODEL_LEN GPU_MEM_UTIL WARMUP KVBD_ARENA_BYTES CONNECTOR_STAGING_GB KVBD_STORE_QUEUE_BYTES KV_BYTES_PER_TOKEN RESULTS_REPO BASELINE_ONLY HF_OVERRIDES KV_CACHE_DTYPE FP8_PREFLIGHT EQUIV_N EQUIV_PROMPT_SET EQUIV_MARGIN_FLOOR EQUIV_GEN_TOKENS MARGIN_CALIBRATE \
