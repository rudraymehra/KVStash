...
**Status:** Mac-loopback shakedown done (Day 2). The real verdict is Day 4 on the AWS c6in pair — these Mac numbers are a **developer-machine sanity check, NOT the gate and NOT a claim.**
...
4. **Bigger frames ≠ faster on loopback** (16 MB slower than 1 MB): again a memory-bandwidth/latency artifact of loopback; on a real link, larger frames amortize syscall cost and help. Re-measure on Day 4.
...
## Next
- Day 4: run `bench/rigs/aws-transport/` on 2× c6in.8xlarge, measure iperf3 ceiling first, grade ≥85% of it. That is the A1 verdict of record.
...
- **[MED] unit mislabel.** `gbps` → `gbytes_per_s` (+ `gbit_per_s` = ×8). The A1 "12 GB/s" = 96 Gbit/s; iperf3 reports Gbit/s — compare correctly on Day 4.
