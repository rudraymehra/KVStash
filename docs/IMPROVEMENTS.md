...
| Spill queue overflow drops under storm are counted but the depth (8) is a guess (2026-07-20) | Needs a measured workload to size against; drops are safe (local copy stays authoritative, retried next pass) | When `kvb_s3_spill_drops_total` is persistently nonzero on a real deployment |
